...
const projections = ['wgs84', 'africa_eqdc'];
...
for (const filename of worldFiles) {
    ...
    for (const projection of projections) {
        const outFileName = filename.replace('admin_0_countries', projection === 'wgs84' ? 'africa' : projection);
        const outFilePath = path.join(regions_path, outFileName);
        exportProjection(
            worldGeoJSON,
            projection,
            false,
            (feature) => feature.properties.NAME !== 'United States of America' && feature.properties.NAME !== 'Kiribati',
            (feature) => feature.properties.REGION_UN === 'Africa' && feature.properties.SUBREGION !== 'Seven seas (open ocean)',
            bboxMap,
            outFilePath
        );
    }
